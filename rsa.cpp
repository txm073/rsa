...
def get_coprimes(n):
    return [i for i in range(1, n + 1) if hcf(i, n) == 1]

def create_charmap(coprimes):
    coprimes = coprimes[1:-1]
    chars = string.printable
    
def get_coefficients(n):
    # Fermat's Little Theorem: mod(e * d, n) = 1
    totient = len(get_coprimes(n))
    e = get_coprimes(totient)[1]
    d = 0
    while True:    
        if e * d % totient == 1:
            return e, d
...
def get_magic_numbers(p, q):
    n = p * q
    d, e = get_coefficients(n)
    return d, e, n, create_charmap(get_coprimes(n))

def encode(public_key, msg):
    return powmod(msg, public_key[1], public_key[0])

def decode(public_key, private_key, msg):
    return powmod(msg, private_key, public_key[0])

p, q = 1000006981, 1000001333
d, e, n, charmap = get_magic_numbers(p, q)
private_key = d
public_key = (n, e)

enc = encode(public_key, 12)
print(enc)
print(decode(public_key, private_key, enc))
*/